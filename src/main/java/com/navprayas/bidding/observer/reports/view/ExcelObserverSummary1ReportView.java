...
public class ExcelObserverSummary1ReportView extends AbstractJExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model,
		...
		WritableSheet sheet = workbook.createSheet("Observer Summary1 Report",
				0);
		...
		sheet.addCell(new Label(colNum++, 0, "Total Sales(Qty X SalesPrice)",
				hdrFmt));
		...
		for (Bids bids : bidsList) {
			...
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidItem()
					.getBidItemId() + ""));
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidItem()
			...
			if (bids.getBidItem().getItemLots().size() == 1) {
				for (ItemLot itemLots : bids.getBidItem().getItemLots()) {
					sheet.addCell(new Label(colNum++, rowNum, itemLots
							.getRemark()));
					sheet.addCell(new Label(colNum++, rowNum, itemLots
							.getLengthRange() + ""));
					sheet.addCell(new Label(colNum++, rowNum, itemLots
			...
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidItem()
					.getTotalQuantity() + " " + bids.getBidItem().getUnit()));

			sheet.addCell(new Label(colNum++, rowNum, bids.getBidItem()
					.getZone()));
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidderName()));
			tempVAlue = (bids.getBidAmount() != null) ? bids.getBidAmount()
					: 0.0;

			sheet.addCell(new Label(colNum++, rowNum, tempVAlue + ""));
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidderName2()));
			tempVAlue = (bids.getBidAmount2() != null) ? bids.getBidAmount2()
					: 0.0;
			sheet.addCell(new Label(colNum++, rowNum, tempVAlue + ""));
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidderName3()));
			tempVAlue = (bids.getBidAmount3() != null) ? bids.getBidAmount3()
					: 0.0;
			sheet.addCell(new Label(colNum++, rowNum, tempVAlue + ""));
			sheet.addCell(new Label(colNum++, rowNum, bids.getBidItem()
					.getIsProcessed()));
			tempVAlue = (bids.getSalesPrice() != null) ? bids.getSalesPrice()
					: 0.0;
			sheet.addCell(new Label(colNum++, rowNum, tempVAlue + ""));
			tempVAlue = (bids.getTotalSalesPrice() != null) ? bids
